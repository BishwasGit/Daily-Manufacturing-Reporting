...
<div class="card-header d-flex justify-content-between align-items-center">
    <h2>{{ label("Sitemenus List") }}</h2>
    <a href="{{ route('sitemenus.create') }}" class="btn btn-primary"><span>{{label("Create New")}}</span></a>
</div>
<div class="card-body">
<table class="table dataTable" id="tbl_sitemenus" data-url="{{ route('sitemenus.sort') }}">
...
{!! getFieldData("tbl_sitemenus", "title", "menu_id", $item->parent_menu) !!}
</td>
<td class="tb-col">
...
<td class="tb-col">
        <div class="dropdown d-inline-block">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-menu-end">
                                                            <li><a href="{{route('sitemenus.show',[$item->menu_id])}}" class="dropdown-item"><i class="ri-eye-fill align-bottom me-2 text-muted"></i> {{label("View")}}</a></li>
                                                            <li><a href="{{route('sitemenus.edit',[$item->menu_id])}}" class="dropdown-item edit-item-btn"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> {{label("Edit")}}</a></li>
                                                            <li>
                                                            <a href="{{route('sitemenus.destroy',[$item->menu_id])}}" class="dropdown-item remove-item-btn" onclick="confirmDelete(this.href)">
...
$(document).ready(function(e) {
    $('.change-alias-badge').on('click', function() {
        ...
        if (isEditing) {
            ...
            var ajaxUrl = "{{ route('sitemenus.updatealias') }}";
            var data = {
                articleId: articleId,
                newAlias: newAlias
            };
