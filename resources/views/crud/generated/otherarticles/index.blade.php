...
<div class="card-header d-flex justify-content-between align-items-center">
    <h2>{{ label("Otherarticles List") }}</h2>
    <a href="{{ route('otherarticles.create') }}" class="btn btn-primary"><span>{{label("Create New")}}</span></a>
</div>
<div class="card-body">
<table class="table dataTable" id="tbl_otherarticles" data-url="{{ route('otherarticles.sort') }}">
...
{!! getFieldData("tbl_otherarticles", "title", "article_id", $item->parent_article) !!}
</td>
...
<td class="tb-col">
        <div class="dropdown d-inline-block">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-menu-end">
                                                            <li><a href="{{route('otherarticles.show',[$item->article_id])}}" class="dropdown-item"><i class="ri-eye-fill align-bottom me-2 text-muted"></i> {{label("View")}}</a></li>
                                                            <li><a href="{{route('otherarticles.edit',[$item->article_id])}}" class="dropdown-item edit-item-btn"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> {{label("Edit")}}</a></li>
                                                            <li>
                                                            <a href="{{route('otherarticles.destroy',[$item->article_id])}}" class="dropdown-item remove-item-btn" onclick="confirmDelete(this.href)">
...
$(document).ready(function(e) {
    $('.change-alias-badge').on('click', function() {
        ...
        if (isEditing) {
            ...
            var ajaxUrl = "{{ route('otherarticles.updatealias') }}";
            var data = {
                articleId: articleId,
                newAlias: newAlias
            };
