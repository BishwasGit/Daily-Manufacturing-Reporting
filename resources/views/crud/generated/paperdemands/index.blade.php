...
<div class="card-header d-flex justify-content-between align-items-center">
    <h2>{{ label("Paperdemands List") }}</h2>
    <a href="{{ route('paperdemands.create') }}" class="btn btn-primary"><span>{{label("Create New")}}</span></a>
</div>
<div class="card-body">
<table class="table dataTable" id="tbl_paperdemands" data-url="{{ route('paperdemands.sort') }}">
...
<th class="tb-col" data-sortable="false"><span
    ...
    <tbody>
        ...
        <tr data-id="{{$item->paperdemand_id}}" data-display_order="{{$item->display_order}}" class="draggable-row">
...
<td class="tb-col">
                        <div class="alias-wrapper" data-id="{{$item->paperdemand_id}}">
                            <span class="alias">{{ $item->alias }}</span>
                            <input type="text" class="alias-input d-none" value="{{ $item->alias }}" id="alias_{{$item->paperdemand_id}}" />
                        </div>
...
<td class="tb-col">
        <div class="dropdown d-inline-block">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-menu-end">
                                                            <li><a href="{{route('paperdemands.show',[$item->paperdemand_id])}}" class="dropdown-item"><i class="ri-eye-fill align-bottom me-2 text-muted"></i> {{label("View")}}</a></li>
                                                            <li><a href="{{route('paperdemands.edit',[$item->paperdemand_id])}}" class="dropdown-item edit-item-btn"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> {{label("Edit")}}</a></li>
                                                            <li>
                                                            <a href="{{route('paperdemands.destroy',[$item->paperdemand_id])}}" class="dropdown-item remove-item-btn" onclick="confirmDelete(this.href)">
...
$(document).ready(function(e) {
    $('.change-alias-badge').on('click', function() {
        ...
        if (isEditing) {
            ...
            var ajaxUrl = "{{ route('paperdemands.updatealias') }}";
            var data = {
                articleId: articleId,
                newAlias: newAlias
            };
