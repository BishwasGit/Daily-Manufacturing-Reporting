...
<div class="card-header d-flex justify-content-between align-items-center">
    <h2>{{ label("Menus List") }}</h2>
    <a href="{{ route('menus.create') }}" class="btn btn-primary"><span>{{label("Create New")}}</span></a>
</div>
<div class="card-body">
<table class="table dataTable" id="tbl_menus" data-url="{{ route('menus.sort') }}">
...
{!! getFieldData("tbl_menus", "title", "menu_id", $item->parent_menu) !!}
</td>
<td class="tb-col">
...
<td class="tb-col">
        <div class="dropdown d-inline-block">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-menu-end">
                                                            <li><a href="{{route('menus.show',[$item->menu_id])}}" class="dropdown-item"><i class="ri-eye-fill align-bottom me-2 text-muted"></i> {{label("View")}}</a></li>
                                                            <li><a href="{{route('menus.edit',[$item->menu_id])}}" class="dropdown-item edit-item-btn"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> {{label("Edit")}}</a></li>
                                                            <li>
                                                            <a href="{{route('menus.destroy',[$item->menu_id])}}" class="dropdown-item remove-item-btn" onclick="confirmDelete(this.href)">
...
$(document).ready(function(e) {
    $('.change-alias-badge').on('click', function() {
        ...
        if (isEditing) {
            ...
            var ajaxUrl = "{{ route('menus.updatealias') }}";
            var data = {
                articleId: articleId,
                newAlias: newAlias
            };
