@extends('backend.template')
        ...
        <h2 class="">{{ label('Edit Menus') }}</h2>
        </div>
        <div class='card-body'>
        <form action="{{route('menus.update',[$data->menu_id])}}" id="updateCustomForm" method="POST" >
 @csrf <input type=hidden name='menu_id' value='{{$data->menu_id}}'/>
<div class="row"><div class="col-lg-6">{{createCustomSelect('tbl_menus', 'title', 'menu_id', $data->parent_menu, 'Parent Menu','parent_menu', 'form-control select2','status<>-1')}}</div><div class="col-lg-6">{{createCustomSelect('tbl_menulocations', 'title', 'menulocation_id', $data->menulocations_id, 'Menulocations Id','menulocations_id', 'form-control select2','status<>-1')}}</div><div class="col-lg-6">{{createText("title","title","Title",'',$data->title)}}
