...
<div class="card-header d-flex justify-content-between align-items-center">
    <h2>{{ label("Units List") }}</h2>
    <a href="{{ route('dmr.units.create') }}" class="btn btn-primary"><span>{{label("Create New")}}</span></a>
</div>
<div class="card-body">
<table class="table dataTable" id="tbl_units" data-url="{{ route('dmr.units.sort') }}">
                            <thead class="table-light">
                                <tr>
                                <th class="tb-col"><span class="overline-title">{{label("Sn.")}}</span></th>
<th class="tb-col"><span class="overline-title">{{ label("units_name") }}</span></th>
...
<th class="tb-col" data-sortable="false"><span
    ...
    <tbody>
        ...
        <tr data-id="{{$item->unit_id}}" data-display_order="{{$item->display_order}}" class="draggable-row">
            <td class="tb-col">{{ $i++ }}</td><td class="tb-col">{{ $item->units_name }}</td>
...
<td class="tb-col">
        <div class="dropdown d-inline-block">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-menu-end">
                                                            <li><a href="{{route('dmr.units.show',[$item->unit_id])}}" class="dropdown-item"><i class="ri-eye-fill align-bottom me-2 text-muted"></i> {{label("View")}}</a></li>
                                                            <li><a href="{{route('dmr.units.edit',[$item->unit_id])}}" class="dropdown-item edit-item-btn"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> {{label("Edit")}}</a></li>
                                                            <li>
                                                            <a href="{{route('dmr.units.destroy',[$item->unit_id])}}" class="dropdown-item remove-item-btn" onclick="confirmDelete(this.href)">
...
$(document).ready(function(e) {
    $('.change-alias-badge').on('click', function() {
        ...
        if (isEditing) {
            ...
            var ajaxUrl = "{{ route('dmr.units.updatealias') }}";
            var data = {
                articleId: articleId,
                newAlias: newAlias
            };
