...
<div class="card-header d-flex justify-content-between align-items-center">
    <h2>{{ label("Deductions List") }}</h2>
    <a href="{{ route('dmr.deductions.create') }}" class="btn btn-primary"><span>{{label("Create New")}}</span></a>
</div>
<div class="card-body">
<table class="table dataTable" id="tbl_deductions" data-url="{{ route('dmr.deductions.sort') }}">
                            <thead class="table-light">
                                <tr>
                                <th class="tb-col"><span class="overline-title">{{label("Sn.")}}</span></th>
<th class="tb-col"><span class="overline-title">{{ label("deduction_amount") }}</span></th>
<th class="tb-col"><span class="overline-title">{{ label("deduction_to") }}</span></th>
<th class="tb-col"><span class="overline-title">{{ label("deduction_from") }}</span></th>
...
<th class="tb-col" data-sortable="false"><span
    ...
    <tbody>
        ...
        <tr data-id="{{$item->deduction_id}}" data-display_order="{{$item->display_order}}" class="draggable-row">
            <td class="tb-col">{{ $i++ }}</td><td class="tb-col">{{ $item->deduction_amount }}</td>
<td class="tb-col">{{ $item->deduction_to }}</td>
<td class="tb-col">{{ $item->deduction_from }}</td>
...
<td class="tb-col">
        <div class="dropdown d-inline-block">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-menu-end">
                                                            <li><a href="{{route('dmr.deductions.show',[$item->deduction_id])}}" class="dropdown-item"><i class="ri-eye-fill align-bottom me-2 text-muted"></i> {{label("View")}}</a></li>
                                                            <li><a href="{{route('dmr.deductions.edit',[$item->deduction_id])}}" class="dropdown-item edit-item-btn"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> {{label("Edit")}}</a></li>
                                                            <li>
                                                            <a href="{{route('dmr.deductions.destroy',[$item->deduction_id])}}" class="dropdown-item remove-item-btn" onclick="confirmDelete(this.href)">
...
$(document).ready(function(e) {
    $('.change-alias-badge').on('click', function() {
        ...
        if (isEditing) {
            ...
            var ajaxUrl = "{{ route('dmr.deductions.updatealias') }}";
            var data = {
                articleId: articleId,
                newAlias: newAlias
            };
